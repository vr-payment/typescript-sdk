...
const transactionService = new VRPayment.api.TransactionService(ApiConfig);
const cardProcessingService = new VRPayment.api.CardProcessingService(ApiConfig);
const tokenService = new VRPayment.api.TokenService(ApiConfig);
const transactionCompletionService = new VRPayment.api.TransactionCompletionService(ApiConfig);
...
describe("TransactionService", () => {
    describe("create()", () => {
        it("should create transaction with given properties", (done) => {
            transactionService.create(ApiConfig.space_id, getTransactionCreate())
                .done((response: { response: http.IncomingMessage, body: VRPayment.model.Transaction }) => {
                    const transaction = response.body;
                    expect(transaction.state).to.equal(VRPayment.model.TransactionState.PENDING);
                    done();
                });
        });
    });
    ...
    describe("confirm()", () => {
        it("should mark transaction as confirmed", (done) => {
            transactionService.create(ApiConfig.space_id, getTransactionCreate())
                .then((response: { response: http.IncomingMessage, body: VRPayment.model.Transaction }) => {
                    const transaction = response.body;

                    const transactionPending = new VRPayment.model.TransactionPending();
                ...
                .done((response: { response: http.IncomingMessage, body: VRPayment.model.Transaction }) => {
                    const transaction = response.body;
                    expect(transaction.state).to.equal(VRPayment.model.TransactionState.CONFIRMED);
                    done();
                });
        });
    });
    ...
    describe("create() with COMPLETE_DEFERRED transaction behaviour", () => {
        it("should make transaction Authorized when we pay with card details", (done) => {
            ...
            transaction.tokenizationMode = VRPayment.model.TokenizationMode.FORCE_CREATION;
            transaction.customersPresence = VRPayment.model.CustomersPresence.NOT_PRESENT;
            transaction.completionBehavior = VRPayment.model.TransactionCompletionBehavior.COMPLETE_DEFERRED;
            ...
            transactionService.create(ApiConfig.space_id, transaction)
                ...
                .done((response: { response: http.IncomingMessage, body: VRPayment.model.Transaction }) => {
                    expect(response.body.state).to.equal(VRPayment.model.TransactionState.AUTHORIZED);
                    done();
                });
        });
    });
    ...
    describe("create() with COMPLETE_IMMEDIATELY transaction behaviour", () => {
        it("should make transaction Filfilled when we pay with card details", (done) => {
            ...
            cardPaidTransaction.tokenizationMode = VRPayment.model.TokenizationMode.FORCE_CREATION;
            cardPaidTransaction.customersPresence = VRPayment.model.CustomersPresence.NOT_PRESENT;
            cardPaidTransaction.completionBehavior = VRPayment.model.TransactionCompletionBehavior.COMPLETE_IMMEDIATELY;
            ...
            transactionService.create(ApiConfig.space_id, cardPaidTransaction)
                ...
                .done((response: { response: http.IncomingMessage, body: VRPayment.model.Transaction }) => {
                    expect(response.body.state).to.equal(VRPayment.model.TransactionState.FULFILL);
                    done();
                });
        });
    });
        ...
        it("should count transactions matching given criteria", (done) => {
            transactionService.create(ApiConfig.space_id, getTransactionCreate())
                .then((response: { response: http.IncomingMessage, body: VRPayment.model.Transaction }) => {
                    ...
                    const criteria: VRPayment.model.EntityQueryFilter = {
                        fieldName: "id",
                        value: transaction.id,
                        type: VRPayment.model.EntityQueryFilterType.LEAF,
                        operator: VRPayment.model.CriteriaOperator.EQUALS
                    };
    ...
    describe("fetchPaymentMethods()", () => {
        it("should fetch payment methods available for the transaction", (done) => {
            transactionService.create(ApiConfig.space_id, getTransactionCreate())
                ...
                .done((response: { response: http.IncomingMessage, body: Array<VRPayment.model.PaymentMethodConfiguration> }) => {
                    const paymentMethods: Array<VRPayment.model.PaymentMethodConfiguration> = response.body;
    ...
    describe("search()", () => {
        it("should find transaction by id", (done) => {
            let transaction: VRPayment.model.Transaction;

            transactionService.create(ApiConfig.space_id, getTransactionCreate())
                .then((response: { response: http.IncomingMessage, body: VRPayment.model.Transaction }) => {
                    transaction = response.body;

                    const criteria: VRPayment.model.EntityQuery = {
                        filter: {
                            fieldName: "id",
                            value: transaction.id,
                            type: VRPayment.model.EntityQueryFilterType.LEAF,
                ...
                .done((response: { response: http.IncomingMessage, body: Array<VRPayment.model.Transaction> }) => {
                    const transactionSearch: Array<VRPayment.model.Transaction> = response.body;
        ...
        it("should successfully update existing transaction", (done) => {
            transactionService.create(ApiConfig.space_id, getTransactionCreate())
                .then((response: { response: http.IncomingMessage, body: VRPayment.model.Transaction }) => {
                    ...
                    const update = new VRPayment.model.TransactionPending();
    ...
    describe("fetchOneClickTokensWithCredentials()", () => {
        it("should return one-click payment tokens (if any) for provided transaction", (done) => {
            transactionService.create(ApiConfig.space_id, getTransactionCreate())
                ...
                .done((response: { response: http.IncomingMessage, body: Array<VRPayment.model.TokenVersion> }) => {
    ...
    describe("processOneClickTokenAndRedirectWithCredentials()", () => {
        let token: VRPayment.model.Token;
        ...
        it("should create URL that can be used to authorize 2nd transaction after we have created token for 1st authorized transaction",
            (done) => {
                const cardPaidTransaction = getTransactionCreate();
                cardPaidTransaction.tokenizationMode = VRPayment.model.TokenizationMode.FORCE_CREATION_WITH_ONE_CLICK_PAYMENT;
                cardPaidTransaction.customersPresence = VRPayment.model.CustomersPresence.NOT_PRESENT;
                cardPaidTransaction.completionBehavior = VRPayment.model.TransactionCompletionBehavior.COMPLETE_DEFERRED;
                ...
                transactionService.create(ApiConfig.space_id, cardPaidTransaction)
                    ...
                    .then((response: { response: http.IncomingMessage, body: VRPayment.model.Token }) => {
                    ...
                    .then((response: { response: http.IncomingMessage, body: VRPayment.model.Token }) => {
                        ...
                        newTransaction.tokenizationMode = VRPayment.model.TokenizationMode.FORCE_CREATION_WITH_ONE_CLICK_PAYMENT;
                        newTransaction.customersPresence = VRPayment.model.CustomersPresence.NOT_PRESENT;
                        newTransaction.completionBehavior = VRPayment.model.TransactionCompletionBehavior.COMPLETE_DEFERRED;
                    ...
                    .then((response: { response: http.IncomingMessage, body: VRPayment.model.TransactionCompletion }) => {
                        expect(response.body.state).to.equal(VRPayment.model.TransactionCompletionState.SUCCESSFUL);
